...
| DEC-041 | **Flash-lite benchmark → provider swap decision (PENDING KHE_AI result).** DEC-002 cost figure = 177đ/extraction-call (per document, NOT per page — output tokens dominate and are ~fixed regardless of page count; input tokens cheap at $0.30/M). Kevin frames cost unit as per-page; KHE_AI to confirm by running 10-page probe and reporting whether output tokens scale. Flash-lite estimated ~31đ/call (5× cheaper). If accuracy ≥90% (M-3 target) → PM ratifies swap. Filed #248. Queue after FE #238 + QC #187 (not on critical path). | **Pending** — awaiting KHE_AI 10-page probe + flash-lite benchmark | 2026-06-24 |
