...
|---|---|
| Phiên bản | v0.9 |
| Trạng thái | Ratified — fold DEC-049 hybrid OCR + DEC-050 R1-R10 EPIC #362 production (cycle 6, PR #402) |
...
- **FR-AU-05 (NEW cycle 6 — Extraction progress polling, Backend PR #374):** `documents.processing_stage` + `processing_progress` (`tenant_020` migration). 4 stages: `ocr` (30%) → `llm` (60%) → `saving` (90%) → `done` (100%). `_mark_failed()` reset progress=0 + stage="failed". Surface trên `DocumentListItem` + `DocumentDetailOut` cho FE polling UX.
