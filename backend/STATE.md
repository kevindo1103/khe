...
| 5b | Reminder service + Telegram + APScheduler | #62 (#26 PR-B, PR #66) | `windsurf/feat-backend-reminder-service` | 🟠 **changes requested** — 5 delivery-reliability gaps: failed-reminder dropped forever, retry dead vs telegram.error.*, time.sleep blocks loop, per-tenant channel_target_ref ignored (global chat), scheduler not guarded by ENVIRONMENT=test |
...
| 6 | Chat query MVP (retrieve-only, D-08) | #27 | `claude/feat-backend-chat-*` | 🟢 **unblocked** — reads `/obligations` + `/documents/{id}/terms` (both live on staging) |
