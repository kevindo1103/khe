...
| 5a | Obligation derivation engine + `/obligations` CRUD | #26 PR-A | `windsurf/feat-backend-obligation-engine` (PR #64) | 🟠 **changes requested** — `derive_obligations` attaches to input `doc_id` not chain terminal → duplicate obligations on confirmed chains (would cascade as duplicate reminders in #62 PR-B) |
