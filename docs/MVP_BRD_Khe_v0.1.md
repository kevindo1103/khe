...
|---|---|
| Phiên bản | v0.11 |
| Trạng thái | Ratified — fold cycle 8 (DEC-056 BA follow-up #493 + DEC-057/058/059 routing + CAIRN cherry-pick prep) |
...
| v0.11 | 2026-07-06 | KHE_Docs | **Cycle 8 fold — DEC-056 BA #493 follow-up + DEC-057/058/059 routing + CAIRN cherry-pick.** §6 Obligation +`source_rule_id`; `source` enum +`"rule_pack"`; `document_id` nullable (Track 2). §6 NEW Virtual Document (DEC-057, type=`rule_pack`, no file bytes). §7.1 FR-IN-06 NEW manual document creation route `/admin/documents/new`. §7.4 FR-OB-18..23 NEW (6 BA #493 sub-decisions ratified Kevin: manual contract keep NOT NULL, series v1 scope, rule pack JSON fixture in repo, TenantProfile extend, luật change → diff+re-confirm D-02, rule-match decline via `rule_activations`). §7.6 FR-CQ note compliance wizard `/admin/obligations/ra-soat` (DEC-059). §7.8 FR-FP note Home 3-CTA layout (DEC-058). |
...
| **Virtual Document** (DEC-057 cycle 8) | Placeholder Document entity với `type="rule_pack"` — không có file bytes, dùng làm anchor cho compliance obligations trong khi `document_id` NOT NULL vẫn active (interim). Cho phép rule pack activate ngay pre-migration. Seeding: 1 virtual doc per tenant per rule pack activation. Không surface trong document list UI (filter `type != "rule_pack"`). |
| **Compliance profile** (BA #493 §4) | Extension trên `TenantProfile` (master.db) cho compliance-source-2. Fields NEW: `legal_form` (loại hình DN), `has_employees` (BOOL), `vat_period` (`month`/`quarter`), `fiscal_year_start` (DATE). Drive rule pack matching (§7.4 FR-OB-18). Wording questionnaire chờ firm pilot validate. |
| **Rule pack v1** (DEC-056 BA #493 §3) | JSON fixture trong repo (`rule_packs/thue_vat_quarterly.json`, etc.) — NOT DB table hoặc CMS pre-pilot. Review-as-code (PR review). Format: `{id, name, applies_when: {legal_form, has_employees, vat_period}, obligations: [{description, cadence, due_expr, ...}]}`. Update via git commit + diff flow (D-02) — NOT auto-apply to existing confirmed obligations. |
| **`rule_activations` table** (BA #493 §6 — NEW per-tenant) | Track rule-match user decision: `id`, `rule_pack_id`, `activated_at`, `activated_by`, `status` (`active` / `declined`), `obligation_ids[]` (obligations sinh từ activation). Decline → NOT re-suggest same rule (avoid spam). Re-activate qua explicit user action (compliance wizard §7.6 DEC-059). |
...
- **FR-IN-06 (NEW cycle 8 — Manual document creation, DEC-058/059):** SME có thể tạo Document metadata thủ công (không upload file) qua route `/admin/documents/new` — cho compliance obligations (không có file HĐ), test scenarios, retroactive entry. Endpoint: `POST /documents` với body `{title, type, contract_number?, parties?, ...}` (no `file_url`). Document tạo với `is_evidence=false` + `type="manual"` (hoặc `type="rule_pack"` cho Virtual Documents §6). No extraction pipeline invoked.
...
**Cycle 8 — DEC-056 BA #493 follow-up (6 Kevin-ratified rules cho compliance obligations source-2):**

- **FR-OB-18 (BA #493 §1 — Manual contract keep `document_id` NOT NULL):** Contracts nhập tay giữ ràng buộc `document_id` NOT NULL — chỉ tax/BHXH/compliance obligations (Track 2) mới được NULL sau khi migration. Interim workaround: DEC-057 Virtual Document (§6).
- **FR-OB-19 (BA #493 §2 — Series v1 scope preserved):** `milestone_series` hiện có (DEC-048) giữ nguyên scope pilot. **KHÔNG** mở rộng series concept cho rule pack obligations trong Sprint 6+. Rule pack obligations sinh mỗi kỳ = discrete row, không group vào series (đơn giản hóa mental model + audit trail).
- **FR-OB-20 (BA #493 §3 — Rule pack v1 JSON fixture in repo):** Rule pack v1 lưu như JSON files trong repo (vd `rule_packs/thue_vat_quarterly.json`). Review-as-code. Update qua git commit + PR review. **KHÔNG** DB table hoặc CMS pre-pilot. Format documented §6 Rule pack v1 entity.
- **FR-OB-21 (BA #493 §4 — `TenantProfile` extend cho compliance profile):** `TenantProfile` (master.db) gain 4 fields: `legal_form` (loại hình DN), `has_employees` BOOL, `vat_period` (`month`/`quarter`), `fiscal_year_start` DATE. Drive rule pack `applies_when` matching. Compliance wizard §7.6 (DEC-059) là entry point để SME điền.
- **FR-OB-22 (BA #493 §5 — Luật thay đổi → diff + re-confirm D-02):** Khi rule pack version update (git commit + PR merge), obligations đã confirm (`status IN {pending, done, awaiting_confirmation}`) **KHÔNG tự động** update. System present **diff view** cho user + require **explicit re-confirm** per obligation (D-02). Un-confirmed rule-pack obligations (status = new state `awaiting_activation`? — Backend TBD) MAY auto-refresh. Prevents silent update destroy audit trail của legal event tracking.
- **FR-OB-23 (BA #493 §6 — Rule-match decline via `rule_activations`):** User decline rule-match suggestion → recorded trong per-tenant `rule_activations` table (§6) với `status="declined"`. System **KHÔNG re-suggest** cùng rule until user explicit re-activate qua compliance wizard. Prevents notification spam + respects D-02 user autonomy.
...
- **FR-FP-04 (NEW cycle 8 DEC-058 — Home 3-CTA layout):** Landing `/admin/` (SME + firm view) hiển thị **3 primary CTAs** kết nối các entry points chính: (1) "Tải HĐ" → `/admin/upload` (DEC-050 flow), (2) "Thêm HĐ thủ công" → `/admin/documents/new` (FR-IN-06 DEC-058), (3) "Rà soát tuân thủ" → `/admin/obligations/ra-soat` (compliance wizard DEC-059). Layout ratified by Kevin, không dark-pattern (D-16 no-paywall-safety).
- **FR-FP-05 (NEW cycle 8 DEC-059 — Compliance wizard standalone route):** `/admin/obligations/ra-soat` là standalone page (không sub-route trong `/documents/`). Wizard step: (1) `TenantProfile` questionnaire (legal_form / has_employees / vat_period / fiscal_year_start), (2) rule pack match preview, (3) per-obligation activate/decline (D-02 confirm), (4) obligations sinh với `source="rule_pack"` + `source_rule_id` + Virtual Document anchor DEC-057. `rule_activations` row per user decision.
...
*Hết v0.11 — Cycle 8 fold (DEC-056 BA #493 6 sub-decisions + DEC-057 Virtual Document + DEC-058 Home 3-CTA + DEC-059 compliance wizard route + CAIRN cherry-pick to CLAUDE.md separate). Bước kế tiếp: KHE_Backend implement rule_activations table + TenantProfile extension + Obligation source_rule_id migration, KHE_Docs finalize compliance questionnaire wording với firm pilot.*
