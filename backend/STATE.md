...
| 5a | Obligation derivation engine + `/obligations` CRUD | #26 PR-A | `windsurf/feat-backend-obligation-engine` | 🟡 **assigned** — kickoff posted; consumes #25 Terms + #50 chain; closes #61 |
| 5b | Reminder service + Telegram + APScheduler | #62 (#26 PR-B) | `windsurf/feat-backend-reminder-telegram` | ⏸ blocked-on PR-A; +APScheduler/python-telegram-bot deps; Event-log idempotency (no migration) |
| 5c | Quota guard (master.db `doc_quota` + ingest dep + monthly reset) | #63 | `windsurf/feat-backend-quota-guard` | ⏸ **triaged & on-hold** — descoped (firm portal removed), sequenced PR-A→PR-B→#63, 6 product Qs pending Kevin/PM |
