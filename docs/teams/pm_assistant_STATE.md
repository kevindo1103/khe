...
*Branch: `claude/pm-assistant` | Last updated: 2026-07-07 | v8.0*

> **2026-07-07 (ad):** **EPIC #493 status update — 14/16 shipped (87.5%), #495 multi-phase tracking.** 4 issues closed today by Kevin: #472 (obligation tab reorg, PR #520 merged), #275 (direction-aware reminders, PR #524 merged), #500 (QC wireframe review done), #466 (design gate tracker complete). Also merged today: PR #521 (backend compliance schema), PR #519 (frontend Home 3 CTA + DocumentNew), PR #527 (ComplianceWizard Step 1 form), PR #523 (D-07 audit drawer mockup), PR #526 (audit drawer frontend), PR #518/#525 (infra project-automation fix). **#495 phase status posted:** AC-1 schema DONE (PR #521), AC-2 activation flow NOT STARTED (hold — firm pilot DEC-013), AC-3 pilot content NOT STARTED (PM/BA + firm). Frontend shipped independently (PR #519/#527) but does not close #495 — remaining backend AC-2/AC-3 require firm content. **Bug filed #529:** `PUT /tenants/me/compliance-profile` 404s for first-time tenants — upsert pattern missing (same as legal_name endpoint). Frontend workaround in PR #527. **Dashboard updated** to 87.5% progress with per-AC phase tracking for #495. **Next:** #529 backend fix (small, assignable), staging→main promotion (7 PRs), then shift focus — obligation pivot largely done, remaining work firm-dependent.
> **2026-07-06 (ac):** **Backlog status audit HOÀN TẤT — 89 open issues → 9 tracks + execution plan.** Kevin yêu cầu "update lại backlog status giúp tôi để triển khai tiếp". Phân tích toàn bộ 89 open issues thành 9 tracks: (1) **Obligation Mgmt Pivot** (EPIC #493, 6 issues — DEC-057/058/059 virtual doc + 3 CTA + compliance wizard, critical path); (2) **DS v1.1 + Document Detail** (EPIC #496, 5 issues — font-serif + Card clause + 33 emoji cleanup); (3) **Two-Pass Extraction** (#443, 9 issues — wiring #461 = P0, risk-routing #462 = P1); (4) **Obligation Engine Core** (#397, 7 issues — #274 StandingDeriver + #275 direction reminders + #276 completeness); (5) **AI/Extraction Quality** (10 issues — hybrid OCR #345, prompt tune, benchmark); (6) **Frontend Polish** (9 issues — journey gates, progress bar, chat reliability); (7) **Infra + Docs + QC** (8 issues — deploy fixes, CAIRN #503, QC automation); (8) **Blocked/Needs Human** (6 issues — DEC pending Kevin); (9) **Deferred/Post-Pilot** (18 issues collapsed — firm portal, marketplace, multi-tenant advanced). **PM recommendation execution order:** (a) close 4 done-staging (#251/#262/#465/#486); (b) resolve 3 journey blockers (#238/#249/#259 → fold into #494); (c) Backend start #494 (virtual doc + manual API); (d) parallel Designer #467/#478; (e) parallel Docs #503 CAIRN cherry-pick; (f) after #494: Frontend new routes #495; (g) after #494: Backend obligation engine #274/#275. **Dashboard artifact published** (interactive HTML, collapsible tracks, stats row).
> **2026-07-06 (ab):** **CAIRN v0.7 evaluation HOÀN TẤT — cherry-pick approach ratified (Kevin).** Đọc toàn bộ CAIRN v0.7 qua branch `cairn-v0.7-review` (Kevin push cairn files vào `docs/cairn-ref/` trên khe repo — git clone bị proxy chặn, GitHub MCP chỉ có scope khe). **Đánh giá: Khế đã có ~80% CAIRN content** (Docs Ownership, topology, Issues bus, lessons-as-rules, bug patterns, deploy, security). **5 delta đáng adopt:** (1) FM-16 role drift — session tự mở scope ngoài role, chưa codify; (2) FM-17 hallucinated artifact — báo committed hash không tồn tại, che FM-16; (3) P-10 post-claim verification — mọi claim done kèm hash/PR URL; (4) spawn templates dispatcher pattern — giảm ramp-up session mới; (5) partial-read discipline — formalize đọc doc theo §anchor. **2 items adopt có điều kiện:** TEAM_STATE YAML schema (chờ ≥3 teams có STATE), machine-readable issue body (chờ Infra cron). **KHÔNG chạy `cairn-upgrade.sh` blind** — script thiết kế cho project mới bootstrap, sẽ clobber Khế CLAUDE.md (500+ dòng domain-specific → generic `{{PLACEHOLDER}}` template), replace NEW_SESSION_INSTRUCTION, tạo `docs/DOCS_INBOX.md` file (Khế dùng GitHub issue #1). **Khế có THỪA so với CAIRN:** D-rules D-01–D-13, Decision Review Gate, DEC registry 59 entries, PR Scope-Lock CI (DEC-047), Multi-Tenant DB, PM_Assistant/Designer/AI/Compliance roles, domain glossary. **Filed #503** (KHE_Docs task-assignment — chi tiết từng item cherry-pick: 3 CLAUDE.md sections + 3 spawn templates customized cho 10-session topology + `.cairn-version` + `docs/CAIRN.md` pointer). DOCS_INBOX #1 relay posted.
> **2026-07-03 (aa):** **Routing architecture analysis HOÀN TẤT — 3 quyết định ratified (Kevin).** Deep analysis toàn bộ routing layer trên staging: `App.tsx` (7 routes), `AppNav.tsx` (nav-lock `FIRST_SESSION_OPEN`), `JourneyContext.tsx` (6 stages), `Home.tsx` (stage-routed views), `Upload.tsx` (AI-only upload), `Settings.tsx` (only legal_name), `tenant_journey.py` (monotonic forward-only, CONFIRMED chỉ từ documents.py), `models/tenant.py` (`Obligation.document_id NOT NULL`). **Phát hiện 2 dead-end:** (1) Tier Sổ user → nav khóa vĩnh viễn vì không có đường tạo document thủ công → không reach CONFIRMED; (2) Compliance-first user → hoàn Flow 3 wizard → journey vẫn NEW → "Nghĩa vụ & Quyền lợi" bị khóa. **Phát hiện DB mới:** `document_id NOT NULL + FK` trên Obligation → mọi obligation PHẢI gắn document → rule-pack obligations cần virtual Document. **3 quyết định ratified:** **DEC-057** virtual Document cho rule-pack (VD Nghị định 70/2025 = record trong `documents` table, type `rule_pack`; obligations gắn document_id của nó; giữ NOT NULL constraint; confirm virtual doc → trigger CONFIRMED via existing Path 2). **DEC-058** Home cold-start 3 CTA: (1) Tải hợp đồng lên → `/admin/upload`, (2) Nhập tay hợp đồng → `/admin/documents/new`, (3) Rà soát tuân thủ → `/admin/obligations/ra-soat`. **DEC-059** Flow 3 wizard = standalone page tại `/admin/obligations/ra-soat` (5-screen wizard, không modal). **Journey spine KHÔNG cần thay đổi** — `advance_stage` đã là ordinal comparison, skip EXTRACTING tự nhiên hoạt động; cả 3 flow converge vào existing `POST /documents/{doc_id}/confirm`. **Folded:** #493 EPIC (full routing analysis + 3 DEC), #494 scope expand (virtual doc seeding + manual doc API), #495 scope expand (2 new routes + nav-lock update + Home 3 CTA), **#502 filed** (Obligations.tsx bug cluster: contradictory trigger button + missing legal basis display), DOCS_INBOX #1 relay.
> **2026-07-03 (z):** **Naming convention mới (Kevin) — dùng tiếng Việt cho copy hiển thị user khi file issue cho design, không lẫn thuật ngữ tiếng Anh như "rule pack".** Chuẩn: `rule pack` → **"gói quy tắc"** trong mọi UI copy (label/hint/nguồn dữ liệu). Sửa 7 chỗ trong wireframe (cột "Nguồn" bảng trạng thái f3-5, hint direction/D-08 ở f3-3/f3-4, thông báo cập nhật luật f3-5). **Ranh giới rõ:** field/tên kỹ thuật trong spec Backend (`source="rule_pack"` enum, tên bảng/biến code) giữ nguyên tiếng Anh — quy tắc chỉ áp dụng cho text user thấy trực tiếp trong UI/design, không áp dụng cho code/schema. **Ghi nhớ cho các lần file issue design sau:** luôn quét lại copy trước khi gửi Designer, không chỉ mockup logic/flow mà cả wording — thuật ngữ kỹ thuật tiếng Anh rất dễ lọt vào khi tôi soạn nhanh từ spec Backend.
> **2026-07-03 (y):** **Kevin: cùng logic nhóm danh mục có nên áp dụng cho f3-4/f3-5 không — có, mở rộng cho nhất quán.** Sau khi nhóm f3-2/f3-3 theo Thuế/BHXH/Lao động ở (x), phát hiện f3-4 (xác nhận D-02) và f3-5 (trạng thái) vẫn phẳng — inconsistent nếu chỉ nhóm 2 màn giữa. Áp dụng nhóm xuyên suốt cả 4 màn: f3-4 dùng `group-header` trong danh sách preview-line; f3-5 dùng sub-header row (`colspan`) trong bảng trạng thái. Không cần field mới ngoài `category` đã đề xuất — chỉ là áp dụng nhất quán ở UI.
> **2026-07-03 (x):** **Kevin đề xuất nhóm card theo danh mục (Thuế/BHXH/Lao động) ở f3-2 + f3-3 thay vì liệt kê phẳng.** Đúng — cần thiết khi rule pack mở rộng vượt 2 luật pilot (DEC-056 vertical mở rộng sau: PCCC/ATTP/môi trường/giấy phép con). Thêm group-header trong wireframe: **Thuế** (GTGT kê khai + hoàn thuế GTGT xuất khẩu — mixed nghĩa vụ/quyền lợi ngay trong 1 nhóm, làm rõ luôn ví dụ vừa sửa ở (w)) · **Bảo hiểm xã hội** (BHXH bắt buộc) · **Lao động** (báo cáo định kỳ). Bổ sung yêu cầu kỹ thuật vào #495: rule schema cần thêm field `category` (domain, độc lập với `document_ref` vì 1 category có thể có nhiều document nguồn — vd Thuế có cả NĐ 70/2025 và Luật Thuế GTGT 2024).
> **2026-07-03 (w):** **⚠️ SELF-CORRECTION (Kevin catch) — rule-pack direction KHÔNG phải "gần như luôn nghĩa_vụ".** Trong #495 + adjudication #500 tôi từng khẳng định rule-pack obligations hardcode `direction="nghĩa_vụ"` ("compliance duties don't run the other way"). Kevin phản ví dụ đúng: **hoàn thuế GTGT** (DN xuất khẩu/dự án đầu tư) là `quyền_lợi` thật, sinh từ CÙNG cơ chế rule-pack như nghĩa vụ thuế/BHXH. Verify lại `Obligation.direction` (`tenant.py:112`) — field vốn agnostic (`nullable=True`, không default cứng) — không cần sửa schema, lỗi hoàn toàn nằm ở giả định sai trong lời văn. **Fix:** rule schema giữ nguyên (`direction` đã là field per-rule từ đầu, đúng); mỗi rule phải tự khai direction riêng khi soạn nội dung (kỷ luật giống D-13 cho nhập tay, không default ngầm). Sửa mockup: thêm dòng "Hoàn thuế GTGT hàng xuất khẩu" (Quyền lợi) song song 2 dòng Nghĩa vụ (GTGT kê khai, BHXH) — minh họa cụ thể trong flow thay vì chỉ khẳng định suông; thêm Q5 hồ sơ tuân thủ (có xuất khẩu không) làm điều kiện match. Đính chính trên #495 (rule schema) + #500 (rút lại phần sai trong adjudication trước). Bài học lặp lại lần thứ N trong session này: đừng khẳng định pattern tuyệt đối ("luôn là X") khi chưa liệt kê phản ví dụ — đặc biệt với domain luật thuế có nhiều loại nghĩa vụ/quyền lợi khác nhau (thuế phải nộp vs thuế được hoàn/miễn/giảm).
> **Cùng ngày, trước đó:** artifact lo-fi từng bị blank page do 1 dòng hint (Flow 2, event-trigger annotation) chứa backtick literal (`` ` ``) lồng bên trong JS template literal cũng dùng backtick — đóng string sớm, vỡ toàn bộ script. Fix: bỏ backtick, giữ plain text tên field. Bài học: khi viết nội dung tiếng Việt có nhắc tên field kỹ thuật trong markdown-style code-format bên trong 1 file HTML dùng template literal, phải kiểm tra không lồng ký tự delimiter.
> **2026-07-03 (v):** **QC review lo-fi wireframe (#500) — 2 vòng, tất cả finding verify đúng, 0 fabrication.** Vòng 1 (7 finding): D-02 checkbox không gate nút xác nhận, thiếu due_date trong readback, direction select tự default (vi phạm D-13 thật), thiếu điều/khoản BHXH, Q4 thiếu escape hatch như Q3, direction ẩn ở Flow 3 (đã confirm hardcode `nghĩa_vụ` trong rule template — chỉ thiếu hiển thị). Fix cả 6, xác nhận điểm 7 không phải bug thật. Vòng 2 (sau khi tôi thêm nghĩa vụ theo sự kiện + màn văn bản pháp luật theo yêu cầu Kevin): QC bắt **1 gap sản phẩm thật, không phải lo-fi** — verify trực tiếp `origin/staging DocumentDetail.tsx`, nút "Sự kiện đã xảy ra" đã tồn tại từ #472 nhưng **hardcode disabled**, tooltip tự khai "cần API xác nhận sự kiện kích hoạt (Backend kickoff riêng)" — chưa từng có issue nào track việc này. Nghĩa là mọi obligation `waiting_trigger` (AI-extraction cũ lẫn nhập tay #494 sắp thêm) hiện KHÔNG có cách đóng. **Filed #501** (Backend — trigger-confirm API), block nhánh "Theo sự kiện" của #494 khỏi Frontend implement tới khi #501 xong; annotate ngay trong wireframe (không chỉ ghi ở GitHub comment) kèm phương án thay thế QC đề xuất (ship Flow 2 v1 chỉ ngày cố định, đo pilot trước). QC cũng bắt lỗi UX thật ở màn "Văn bản áp dụng": card dẫn bằng số hiệu nghị định thay vì tên chính sách ngôn ngữ thường — CEO nhận diện "hóa đơn điện tử"/"đóng BHXH" chứ không nhận diện "70/2025/NĐ-CP". Fix cả 3 card, đổi hierarchy title/caption. Bài học: QC hoạt động độc lập, tự verify code thật thay vì tin lo-fi tại mặt — 2/2 vòng đều bắt được ít nhất 1 gap PM tự làm bỏ sót (round 1: D-13 select-default; round 2: dead API stub có sẵn trong code).
> **2026-07-03 (u):** **BA tổng thể "User flow toàn bộ Obligation OS" delivered + 6 quyết định ratified (Kevin "Đồng ý cả 6 điểm") + lo-fi wireframe dựng.** Kevin yêu cầu cụ thể: unified flow cho Nghĩa vụ từ hợp đồng (nhập tay/AI) + Nghĩa vụ thuế/BHXH (khai báo/check/xác nhận/database setup). Delivered: 5-stage lifecycle chung (Source→Derive→Direction→Confirm D-02→Track) áp cho cả 3 nguồn; Flow 1 (AI, baseline không đổi); Flow 2 (nhập tay — làm rõ quan trọng: **manual contract VẪN có `document_id`**, không NULL — vì có Document record thật dù chỉ metadata; NULL chỉ dành riêng tax/BHXH); Flow 3 (rule pack — trả lời đủ 4 câu hỏi Kevin: khai báo qua compliance-profile 4-5 câu ở Settings, check qua rule-matching engine so khớp điều_kiện, xác nhận kèm căn cứ pháp lý D-02, database = JSON fixture rule pack curated trong repo — KHÔNG AI đọc luật trực tiếp, tránh D-01). **6 quyết định ratified:** (1) manual-contract giữ document_id, NULL chỉ cho tax/BHXH (2) series v1 scope giữ nguyên, không mở rộng rule pack pilot (3) rule pack JSON fixture trong repo, review-as-code, không DB table (4) `TenantProfile` là nơi mở rộng field hồ sơ tuân thủ (5) luật đổi → diff+re-confirm (D-02), không auto-update (6) dòng decline không re-suggest, track qua `rule_activations` mới. Đã **fold vào GitHub**: comment chi tiết trên #493 (BA đầy đủ), #494 (làm rõ ranh giới document_id + field mới `source_rule_id`), #495 (spec kỹ thuật Flow 3: compliance profile fields, rule schema điều_kiện/nghĩa_vụ/lịch/direction/phạt/căn_cứ, `rule_activations` table, preview-có-căn-cứ UX, diff flow) + DOCS_INBOX #1 relay (schema/architecture implications). **Lo-fi wireframe** dựng xong (HTML artifact, xám/không màu chủ đích để feedback tập trung flow/wording không phải UI): 3 màn Flow 2 (metadata hợp đồng → nhập nghĩa vụ từng dòng → preview/confirm D-02) + 4 màn Flow 3 (hồ sơ tuân thủ → kết quả rà soát có căn cứ pháp lý → xác nhận D-02 → trạng thái theo dõi sau kích hoạt). Click-through, step-rail hiển thị tiến trình. **Còn mở:** wording compliance-profile questionnaire (Flow 3 màn 1) cần validate với firm đại lý thuế pilot DEC-013 trước khi lock — flag rõ trong mockup bằng annotation. Backend #494 có thể chạy song song ngay (API shape đã ratify, không phụ thuộc lo-fi); chỉ Frontend + rule-pack content chờ Kevin xem lo-fi.
> **2026-07-03 (t):** **#494 bổ sung — `direction` bắt buộc khi `document_id IS NULL`.** Verify `directions.py`: cơ chế D-13 bắt buộc `document_id` để query `Party` theo doc rồi so khớp `legal_name` — nghĩa vụ nhập tay/rule-pack không có document_id → không có Party để so khớp → direction luôn NULL vĩnh viễn nếu để tự suy đoán, vỡ IA #466 (chia trục Nghĩa vụ/Quyền lợi). Fix: `direction` thành field bắt buộc trong `POST /obligations` khi thiếu document_id — nhập tay user tự chọn, rule pack đóng cứng trong template (hầu như luôn `nghĩa_vụ`). Posted vào #494. Không cần BA lại tổng thể Obligation OS — DEC-056 + EPIC #493 đã đủ chắc, đây chỉ là 1 gap kỹ thuật cụ thể phát hiện khi verify trước khi trả lời câu hỏi đó.
> **2026-07-03 (s):** **Audit DS v1.1 toàn bộ 10 trang admin — EPIC #496 + sub-issue #497 filed.** Nguyên nhân Kevin hỏi "sao text chưa đổi" ở tab Nội dung hợp đồng: PR #488 chỉ fix 4 điểm hẹp (green-creep, orphan panel, tint) từ #478, KHÔNG phải revamp toàn bộ theo `mockup_document_detail_v4.jsx` — verify code thật: `ClauseTreeItem`/`ClauseContent` vẫn phẳng, không Card. **Phát hiện nền tảng: `font-serif` = 0 trên TOÀN BỘ 10 trang**, `tailwind.config.js` chưa khai báo — nguyên tắc D-06 (chữ hợp đồng≠chữ phần mềm) chưa hề tồn tại trong code dù ratify từ #467/#469. Audit đầy đủ bằng grep thật (bg-success/bg-done/border-strong/font-serif/emoji python-unicode) cho cả 10 trang: chỉ `DocumentDetail` (tab Nghĩa vụ) + `DocumentList` thật sự đã revamp; `DocumentDetail` (3 tab còn lại) chỉ fix lẻ; 7 trang còn lại (`Obligations.tsx` dashboard tenant thật — route `/obligations` riêng, không phải tab — `Settings`/`Upload`/`ExtractionMetrics`/`Home`/`Chat`/`Login`) chưa chạm, chưa có mockup. **Nguyên tắc Kevin ra:** pivot EPIC #493 đụng màn nào thì revamp màn đó luôn, không big-bang 10 trang. Phân 3 nhóm: (1) có mockup làm ngay → #497 (Frontend, font-serif + Card cho clause tree + dọn 33 emoji) (2) sẽ bị #493 đụng (Settings, màn tạo tay mới, Obligations dashboard) → mockup+revamp khi tới lượt, không file trước (3) không liên quan pivot (Home/Upload/ExtractionMetrics/Chat/Login) → để sau.
> **2026-07-03 (r):** **BA + backlog tổng ratified — EPIC #493 + sub-issue #494/#495 filed.** Phát hiện cấu trúc: nhập tay (DEC-055) + rule pack compliance (DEC-056) KHÔNG phải 2 luồng song song — cả hai đều là "tạo Obligation không cần document", rule pack chỉ là "nhập tay hàng loạt theo template". Đối chiếu #274/#275/#276: #274 (StandingObligationDeriver) + #276 (completeness flag) chỉ áp dụng track AI-extraction; #275 (direction-aware reminders) là engine dùng chung cho MỌI nguồn nghĩa vụ. **Q3 (ai được tạo Obligation) tự giải quyết khi Kevin loại firm khỏi scope phase này** — rule pack activation là tính năng nội bộ SME, không qua firm → D-09 giữ nguyên, không cần sửa, không cần consent gate mới (tự nhận lỗi: D-12 đề xuất trước đó ở DEC-056 xung đột ngầm với D-09 mà tôi bỏ sót lúc soạn — Decision Review Gate lẽ ra phải bắt được). **Cấu trúc 3 tầng ratified:** Tầng 0 (nền tảng — `POST /obligations` không cần document, `Obligation.document_id` nullable, `source` enum thêm `"rule_pack"`) + Track 1 (#275 song song Tầng 0, #274/#276 độc lập) + Track 2 (rule pack schema + nội dung pilot thuế/BHXH với đại lý thuế DEC-013, đặt SAU Tầng 0). Filed: **#493** (EPIC) · **#494** (Tầng 0, sub-issue) · **#495** (Track 2, sub-issue) — đã link sub-issue vào EPIC.
> **2026-07-03 (q):** **Priority pivot (Kevin) — Chat KHÔNG còn là tính năng ưu tiên, tập trung obligation management.** Khớp thẳng DEC-056 (Obligation OS North Star, không phải conversational interface). #277 (chat reliability, D-08 false-positive) hạ ưu tiên — KHÔNG đóng (bug thật, vẫn tồn tại), bỏ `blocker:human-needed`, giữ `status:planned` để không mất dấu, không ai nhặt cho tới khi chỉ định lại. **Hàng đợi Backend chủ động mới:** #274 (StandingObligationDeriver) → #275 (direction-aware reminders) → #276 (completeness flag) — cả 3 đều obligation-management core, ready-to-code. **Câu hỏi sequencing chưa hỏi Kevin:** #274-276 (đào sâu obligation engine hiện có) trước, hay DEC-056 rule pack (mở rộng nguồn nghĩa vụ compliance) song song/trước? Cả hai đều "obligation management" nhưng khác trục — cần Kevin xác nhận thứ tự khi rảnh.
> **2026-07-03 (p):** **DS v1.1 rollout HOÀN TẤT — đóng #489/#485/#481.** PR #491 (claude_dev, không phải windsurf_dev — giải thích branch name lệch `claude/feat-frontend-metrics-dashboard-kd76vr` không theo pattern windsurf) verify 4/5 đúng, 1 note nhỏ không chặn (`sortDocs` không gọi `isPendingDoc()` trực tiếp, tự viết lại logic tương đương — rủi ro drift nhỏ, Kevin quyết để nguyên). Grep staging xác nhận sạch hoàn toàn `bg-success`/`text-success` trên cả `DocumentDetail.tsx` + `DocumentList.tsx`. #481 (BA gốc 3 gap) đóng — DS v1.1 nhất quán trên 4 tab doc-detail + document list. **Còn treo, ngoài phạm vi vừa đóng:** Q-Width (chờ Kevin chọn giá trị `AdminShell.tsx`), Backend #274/#275/#276/#277 (P1 pre-pilot, đứng yên từ Sprint 1, **#277 chat reliability là 🔴 blocker:human-needed, Kevin tự UAT verdict "chưa thể cho user thật test" — đáng ưu tiên nhất, chưa ai đụng**), DEC-056 compliance rule pack spec (chờ docs fold DEC-056 trước).
> **2026-07-03 (o):** **QC #489 verify đúng — 4 chỗ green-creep sót lại trong `DocumentDetail.tsx`, PR #488 chưa touch.** Grep `bg-success|text-success` trên staging + đối chiếu diff thật PR #488 (còn mở): `StatusBadge` obligation status="done" (~151), `fulfilled_at` badge (~404), `SeriesCard` next-installment amount (~474), "Đã hoàn thành" section header dot+badge (~1458-1459) — cả 4 xác nhận KHÔNG nằm trong diff #488. Post fix cụ thể lên PR #488 để fold trước merge (rẻ hơn follow-up PR, token `done`/`done-soft` đã có sẵn). **1 điều chỉnh quan trọng so với đề xuất QC:** dòng ~474 (SeriesCard amount) không nên đổi `text-success`→`text-done` như 3 chỗ kia — đây là cùng bug ngữ nghĩa với `AmountDisplay` vừa fix (amount=dữ kiện, không phải trạng thái) → phải đổi thành `text-ink`, không phải `text-done`, nếu không chỉ đổi màu mà chưa sửa đúng gốc. Đúng điều tôi lo khi ratify Q1 ở #478 ("gộp 1 lần tránh sửa lẻ tẻ") — QC bắt đúng lúc trước merge.
> **2026-07-03 (n):** **DEC-056 RATIFIED (Kevin "đồng ý hoàn toàn") — Obligation OS. Issue #486 + DOCS_INBOX relay posted.** North Star mới: Servanda = "Obligation OS" không phải "Contract OS" — hợp đồng là nguồn nghĩa vụ THỨ NHẤT, **pháp luật là nguồn THỨ HAI** (thuế/BHXH/ngành dọc PCCC-ATTP-môi trường-giấy phép con). Luận điểm Kevin: CEO phụ thuộc kế toán để nhớ nghĩa vụ thuế (single point of failure), chi phí vi phạm luật định > contract breach, TAM = mọi DN. PM điều chỉnh trung thực: KHÔNG hoàn toàn xanh (MISA/eTax có nhắc thuế) — khe hở thật = (a) sổ cái HỢP NHẤT cho CEO, (b) portfolio view cho firm, (c) ngành dọc (xanh thật). **Kiến trúc:** rule pack curated theo hồ sơ DN + nhập tay (rail DEC-055 sẵn); KHÔNG AI-đọc-luật (D-01/D-08); **D-12 mới đề xuất**: firm kích hoạt+xác nhận lịch tuân thủ (D-02), Servanda không tư vấn luật — lá chắn trách nhiệm kiêm product design. **Khớp chiến lược:** DEC-011 (đại lý thuế = kênh, việc của họ thành product), DEC-018 reframe (wedge = loại-nghĩa-vụ compliance cắt ngang ngành, không phải 1 ngành), DEC-055 (rule pack = structured data thuần không AI call, GM đẹp, recurring revenue tự nhiên khi luật đổi). **3 gaps:** document_id NOT NULL cần nới · rule engine lịch luật định · vận hành nội dung = chi phí VĨNH VIỄN lớn nhất. **Sequencing:** KHÔNG chen Sprint 2; validate = 1 rule pack (thuế+BHXH) với đại lý thuế DEC-013; backlog kỹ thuật chỉ mở sau docs fold + firm pilot xác nhận giá trị. OPEN: vị trí rule pack trong tier (gắn DEC-016).
> **2026-07-03 (m):** **2 quyết định RATIFIED (Kevin, ủy quyền PM quyết trực tiếp).** (1) **Q1/Q1-ext — bỏ xanh lá "success" ở cả 4 vị trí:** `ConfidenceMeter` (≥ngưỡng→`ink-faint`) · `SignatureBadge` "Đã ký"→`done` · `LifecycleBadge` "active"→`ink-muted` outline · `AmountDisplay`→chữ `ink` thường bỏ badge. Không phải quyết định mới — áp dụng nhất quán nguyên tắc v1.1 đã ratify từ đầu ("hoàn thành=xám lặng"), 4 chỗ chỉ là drift qua nhiều PR. Code đã có sẵn trong PR #480/#483 (non-legacy variant), không cần mockup mới. (2) **Q-Status-Failed — xác nhận là bug thật, cần fix logic:** doc `status="failed"` phải tách khỏi "Cần xác nhận" (khác hành động user cần làm) — thêm state "Lỗi trích xuất" đỏ, dùng `isPendingDoc()` helper đã có. Posted ratify tại [#478](https://github.com/kevindo1103/khe/issues/478#issuecomment-4875185998) + [PR #483](https://github.com/kevindo1103/khe/pull/483#issuecomment-4875186489), gỡ block cho #485 (Frontend làm luôn cả đổi màu ở Phase 1). Còn treo không đổi: content_status/DocumentRelationship (chờ #484 Backend + Kevin scope), Q-Width (chờ giá trị cụ thể).
> **2026-07-03 (l):** **PR #480 (doc-detail v4, #478) MERGED — review sau (retroactive), chất lượng xuất sắc.** Designer research vượt brief: phát hiện `Term.source` có 3 giá trị không phải 2, field tên thật `contract_term` không phải `contract_duration` (CLAUDE.md cũng sai), `content_status` có 4 giá trị thật (`NULL/skeleton/filled/truncated`) 0% API/FE, `clause_count` không phải cột DB mà COUNT query. **PR #483 (documents-list v3, #481) reviewed — verify 6/6 claim đúng 100%** (StatusPill thiếu nhánh "failed" = bug thật, `duplicate` field không tồn tại, `may_have_unextracted_obligations` hardcode None, list API thiếu `doc_type_group`, `LifecycleBadge.tsx` cũng xanh lá — instance thứ 3, scope 2 file đúng DEC-047). Designer tự QC phát hiện bug tinh vi trong chính PR (toggle màu không propagate vào filter thật) → tự sửa bằng `isPendingDoc()` dùng chung. **Tổng hợp quan trọng: màu xanh lá "success" giờ lộ ở 4 vị trí** (ConfidenceMeter, SignatureBadge, LifecycleBadge, AmountDisplay) qua nhiều PR — đề xuất Kevin chốt 1 lần ở #478, không sửa lẻ tẻ. **Filed #484 (Backend — thêm `Term.source` vào TermOut API, blocker cho badge "Nhập tay"; input chuẩn bị cho content_status/DocumentRelationship, chờ Kevin ratify scope) + #485 (Frontend — Phase 1 implement #480 ngay/Phase 2 chờ merge #483, KHÔNG tự đổi màu trước khi Kevin chốt gộp).**
> **2026-07-03 (k):** **DOCS_INBOX relay — 3 điểm từ andrej-karpathy-skills fold vào CLAUDE.md.** Kevin chỉ đề xuất tham khảo repo ngoài (4 nguyên tắc: Think Before Coding/Simplicity First/Surgical Changes/Goal-Driven Execution). Đối chiếu: phần lớn đã trùng Decision Review Gate/Bug Fix Protocol/Anti-Patterns hiện có. Cài plugin (`/plugin marketplace add...`) là quyết định môi trường cá nhân Kevin, KHÔNG phải việc PM chạy thay — đã giải thích rõ, không tự ý cài. Chỉ relay 3 delta thật sự mới: (1) §Decision Review Gate — trình bày nhiều cách hiểu hợp lệ thay vì tự chọn ngầm (khác case conflict-DEC đã có); (2) §Bug Fix Protocol — biến task thành tiêu chí verify được (viết test trước khi sửa), hữu ích Backend/QC; (3) §Anti-Patterns — chỉ xóa import/dead-code do CHÍNH thay đổi mình gây ra, không tự xóa dead code có sẵn. Posted DOCS_INBOX #1.
> **2026-07-03 (j):** **QC UI review tab Nghĩa vụ & Quyền lợi trên staging (doc #6, #472 đã deploy) — 4 lỗi, verify trong code thật `DocumentDetail.tsx` origin/staging.** 🔴 P1: (1) "5 đ" — `formatCurrency` line ~122 strip dấu `%` từ `amount_raw="5%"` → "5 đ", sai lệch dữ liệu tài chính (thực = 5% giá trị căn hộ); fix = detect % → return null (đúng Q5). (2) "Hạn: Invalid Date" — `dueLabel`/`overdueLabel` line ~104-117 + `temporalBucket` thiếu guard `isNaN(new Date())`; date lạ → "Invalid Date" + rơi nhầm bucket "Sắp tới". 🟡 P2: (3) `AmountDisplay` line ~170 `bg-success-soft text-success` xanh lá — instance thứ 3 của green-creep (cùng loại ConfidenceMeter/SignatureBadge #478). (4) emoji ✅ line ~2588 banner xác nhận, vi phạm Q7 (đã có Badge "đã xác nhận" ngay trước → thừa). Report → #472. Cần Backend xác nhận `amount_raw` chứa gì cho đợt %. Reorg IA/series/bulk-select bản thân chạy tốt — chỉ 4 lỗi trên. Ghi chú: `✓` trong toast/checkbox là borderline chấp nhận (checkmark chức năng), chấm màu section header OK (color+text pair).
> **2026-07-03 (i):** **Filed #478 — Designer làm `docs/mockup_document_detail_v4.jsx`** (3 tab: Tổng quan/Nội dung hợp đồng/Bên ký kết — KHÔNG đụng tab Nghĩa vụ & Quyền lợi, đã có #467/#468/#472). Nguồn: PM dựng prototype HTML grounded trên code thật (2 vòng Explore agent verify `DocumentDetail.tsx` + model `Term`/`Party`/`Clause`/`Document`) + nhiều vòng feedback trực tiếp Kevin, chốt 4 fix: bỏ tint xám vô cớ card "Thời hạn hợp đồng" · label field đậm `ink` thay `ink-muted` (dễ quét) · rẽ nhánh `Term.source` — manual→badge "Nhập tay" (đúng variant outline, không tự chế class như prototype PM mắc lỗi lần đầu) thay `ConfidenceMeter` · panel cross-ref cap chiều cao + modal xem đầy đủ (case thật 14 tham chiếu doc #14). **3 quyết định mở đóng khung riêng, không để Designer tự quyết:** (1) `ConfidenceMeter`+`SignatureBadge` đang xanh lá "success" trên production, mâu thuẫn triết lý v1.1 "hoàn thành=xám lặng" — đổi hành vi production thật; (2) `content_status` (two-pass #443) hoàn toàn chưa có UI, khác cơ chế "stub" hiện tại (`node.content==='(tổng hợp từ mục con)'`); (3) `DocumentRelationship` (annex/amends R4a) không có UI nào — phát hiện phụ. Bắt buộc Designer dùng component/token từ `mockup_design_system_v1.1.jsx` (PR #473), không tự định nghĩa CSS var riêng. **Ngoài lề đáng nhớ:** 1 "báo cáo nghiên cứu" dài do Kevin gửi review DS v1.1 — verify lần 1 (nhầm file, check artifact PM thay vì file Designer) kết luận sai là "phần lớn bịa"; verify lần 2 đúng file Designer (`design_system_v1.1_preview.html`, companion preview của JSX #473, Kevin xác nhận không phải nguồn song song) cho thấy hầu hết chi tiết ĐÚNG, chỉ 2 kết luận trung tâm của lộ trình khuyến nghị (thiếu semantic HTML, thiếu @media) là sai — cả hai đều verify được bằng grep trực tiếp. Bài học: luôn xác nhận ĐÚNG file trước khi kết luận, kể cả khi đã có prior similar case (Copilot review PR #468) khiến dễ pattern-match nhầm "chắc lại bịa nữa".
> **2026-07-03 (h):** **Gate #466 ĐÓNG HOÀN TOÀN — PR #474 merge, verify thật trên staging (commit `6b23e1b`).** Badge Phạt giờ đúng viền đỏ/nền trong (`fg={t.color.danger} border={t.color.danger}`), khớp yêu cầu lần 2. Chuỗi merge cuối `docs/mockup_obligation_tab_v3.jsx`: 475573f (reorg gốc) → 6898cce (dedupe series) → 255f72a (QC fix: nullDir overlap + bỏ glyph ↗ vi phạm zero-icon) → 6b23e1b (Phạt outline, PR #474). Cả 2 mockup nguồn (`mockup_obligation_tab_v3.jsx` behavior/IA + `mockup_design_system_v1.1.jsx` token/component, PR #473) giờ khớp nhau hoàn toàn — không còn sai lệch cần Frontend tự nhớ. **Đã báo bắt đầu sprint:** #471 (Backend — bulk endpoint, penalty enum, điều tra self-party doc #14) + #472 (Frontend — implement reorg, import token từ v1.1 file, 1 quyết định mở không chặn: modal xác nhận bulk-complete). Không còn blocker nào. Bài học quy trình rút ra qua vòng #468 dài này: **luôn verify bằng diff/staging thật trước khi báo "đã fix"** — 2 lần trước đó lời báo "đã fix" không khớp code thật, lần này khớp.
> **2026-07-03 (g):** **PR #473 merged (Kevin) — `docs/mockup_design_system_v1.1.jsx` chính thức trên staging.** #470 đóng. Update #472 (Frontend): import trực tiếp file thật (18 component + tokens) thay vì bảng token trong comment #467 — file là nguồn ưu tiên nếu có sai khác. Nhắc riêng: `Badge kind="penalty"` trong file đã outline sẵn, Frontend copy thẳng, không cần tự dựng. Còn mở: PR #468 (mockup obligation tab) chờ Designer patch badge Phạt riêng (yêu cầu tại PR #468 comment) — chưa xong tại thời điểm này.
> **2026-07-03 (f):** **PR #473 review — #470 DS v1.1 formalization hoàn thành, chất lượng cao.** Verify bằng diff thật (không tin mô tả PR): token 100% khớp #467/#469 (hex + tỷ lệ contrast đúng từng số), elevation e0-e3 đúng, badge `penalty` **đã outline ngay từ đầu** (đúng điểm vừa yêu cầu Designer sửa ở PR #468 — file #473 làm đúng tự nhiên, không cần nhắc), 11 `kind` cover đủ 13 label vocabulary (2 cặp share kind hợp lý: Hôm nay/Còn-N-ngày→dueSoon, Chờ-kích-hoạt/Chờ-xác-nhận→waiting), đủ 4 component A11y #206 (NavItem/IconButton/Dropzone/LiveRegion) + bonus VisuallyHidden, `IconButton` dev-warn thiếu label + tự nhắc "không dùng cho status, dùng Badge". Scope đúng 2 file (mockup + designer_STATE.md), KHÔNG retroactive lên PR #468. `designer_STATE.md` ghi thẳng lịch sử "messy" (v1.0→PM retract sai→Kevin override→v1.1) — trung thực, không né. Không tìm thấy vấn đề chặn merge — recommend merge sau Kevin review cuối. Update gate #470.
> **2026-07-03 (e):** **Fix-at-source thay vì Frontend tự deviate.** Kevin chỉ đạo: badge "Phạt" sai (nền đặc thay vì viền, kế thừa v0.2) phải sửa NGAY tại `mockup_obligation_tab_v3.jsx` (PR #468) — không để Frontend tự nhớ patch khi implement, vì mockup này sẽ là tham chiếu cho các task nghĩa vụ sau, sai ở nguồn dễ bị copy lặp. Đã request Designer patch cụ thể (1 dòng, `TextBadge` đã sẵn prop `border`) qua [PR #468 comment](https://github.com/kevindo1103/khe/pull/468#issuecomment-4872851844). Update #472 (Frontend) bỏ phần "tự deviate", giờ chỉ cần copy mockup sau khi patch xong. Nguyên tắc rút ra: **quy tắc ngữ nghĩa trạng thái (§04 badge vocabulary) áp dụng ngay bất kể đang grandfather token nào** — khác với raw hex/font/radius (được phép giữ nguyên v0.2 cho PR đã duyệt).
> **2026-07-03 (d):** **Gate #466 MỞ — Dev kickoff filed.** Kevin yêu cầu re-check PR #468 rồi file Backend+Frontend trực tiếp (QC verify bước 3 gộp vào Kevin review, không tách riêng theo chỉ thị hôm nay). Re-check: PR #468 không đổi chức năng gì thêm — 3 commit = (1) reorg gốc, (2) dedupe series-card fix (đã có TRƯỚC lúc PM review lần đầu, không phải mới), (3) `designer_STATE.md` ghi lại DS v1.1 spec đầy đủ + chính xác (Designer tự transcribe từ #467/#469, khớp bảng token PM relay). **Gap cũ vẫn mở:** `handleBulkComplete` chưa qua modal xác nhận D-02 — verify bằng diff thật (grep `handleBulkComplete`), không đổi. Chuyển thành quyết định mở trong kickoff Frontend (không chặn, vì endpoint Backend không đổi hình dạng dù có/không modal). **Filed:** #471 (Backend — bulk PATCH endpoint + `penalty` enum migration + điều tra root cause self-party fail doc #14: legal_name chưa set hay `Party.aliases` chưa vào `_self_party_strings()`) · #472 (Frontend — implement mockup vào `DocumentDetail.tsx`, **flag rõ dùng token v1.1 KHÔNG PHẢI v0.2** mockup đang dùng: Phạt phải viền-đỏ-nền-trong không nền-đặc, viền tương tác dùng `border-strong` không `n-300`). Gate tracker #466 update. Migrate-on-touch: xóa nốt emoji `SelfPartyConfirmed`/`CompletenessBanner` gộp vào #472 cùng lượt chạm file.
> **2026-07-03 (c):** **Filed #470 — KHE_Designer: chính thức hóa DS v1.1 thành `docs/mockup_design_system_v1.1.jsx`.** Lý do: DS v1.1 hiện chỉ tồn tại dưới dạng artifact chat (không git-tracked, session khác không tự đọc được, PM không được ghi vào `docs/mockup_*.jsx` — lane của Designer). Task: tạo file sống theo format `mockup_design_system_v0.2.jsx` hiện có, nội dung từ spec đã ratify (#467/#469), **fold nguyên A11y contract #206 + 4 component mới** (NavItem/IconButton/Dropzone/LiveRegion), **KHÔNG tái dùng token màu/font/radius của v0.2** (v1.1 có bộ riêng, xem #467). v0.2 = superseded cho mockup mới, KHÔNG cần re-migrate PR #468 hay mockup cũ (theo policy migrate-on-touch có sẵn).
> **2026-07-03 (b):** **#469 RESOLVED — Kevin ratified: giữ v1.0 canonical, nâng thành v1.1** (không rút lại như PM đề xuất ban đầu). Fold 3 phần từ DS v0.2 vào v1.1: (1) **Contrast đo thật** (WCAG 2.1 relative luminance, tự tính bằng python không suy đoán) — phát hiện gap thật trong v1.0 gốc: `n-300`(1.46:1)/`n-400`(2.08:1) KHÔNG đạt 3:1 cho biên input/button (WCAG 1.4.11) → thêm token mới **`border-strong` #7E8983 (3.47:1)**, `n-200/n-300` giữ lại chỉ cho viền trang trí. (2) **Hợp đồng A11y** fold nguyên #206: semantic-element mandate + keyboard-operable + LiveRegion + icon-only bắt buộc aria-label; 4 component mới `NavItem`/`IconButton`(ngoại lệ duy nhất luật không-icon)/`Dropzone`/`LiveRegion`. (3) **Elevation 4-tầng** e0-e3 (ẩn dụ "giấy xếp lớp", khớp triết lý Sổ cái) thay 1 bóng phẳng. Artifact republish v1.1 (label `v1.1-fold-v0.2`, cùng URL). **Relay:** [#469 đóng](https://github.com/kevindo1103/khe/issues/469#issuecomment-4872667189) · [#467 đính chính lần 2](https://github.com/kevindo1103/khe/issues/467#issuecomment-4872669400) (v1.1 = chuẩn chính thức mọi mockup từ đây, PR #468 cũ không cần sửa vì token) · [DOCS_INBOX #1](https://github.com/kevindo1103/khe/issues/1#issuecomment-4872671495) (THAY THẾ entry v1.0 07-02, không giữ song song). **Ngoài lề:** Copilot review PR #468 (Kevin dán trực tiếp, test tính năng Git, không phải comment GitHub thật — verified `get_reviews`/`get_comments` rỗng) — phát hiện 2 claim BỊA (dòng ~930-960 không tồn tại, file chỉ 868 dòng; "data bị cắt series_to[...]" — đọc lại nguyên văn không hề cắt) + 1 hiểu sai spec (amount-hide là Q5 ratified, không phải bug) + 2 điểm nhỏ hợp lệ (checkbox tooltip, Settings href chưa wire — bình thường vì mockup tĩnh). Kevin xác nhận không cần action, chỉ là test — không post gì thêm.
> **2026-07-03 (a):** **⚠️ SELF-CORRECTION — "Servanda DS v1.0" (2026-07-02f) SAI, mâu thuẫn DS v0.2 canonical (DEC-017).** Phát hiện khi review PR #468 (Designer mockup #467): PR dùng "DS v0.2 canonical" — hóa ra **v0.2 đã ratify TRƯỚC** (DEC-017, migrate 7/7 màn hình admin, a11y contract binding #206, contrast ĐO ĐẠC THẬT không như v1.0 chỉ khẳng định suông). PM đã soạn v1.0 **KHÔNG đọc DEC-017 trước** — vi phạm chính Decision Review Gate checklist của mình. Bằng chứng token trôi rộng hơn: `designer_STATE.md` (#378, trước #467) từng dùng **palette thứ 3** khác cả v0.2 lẫn v1.0 — token hygiene đã lỏng từ trước, không chỉ lỗi hôm nay. **PR #468 KHÔNG sai** — Designer đúng khi bám v0.2 thật, bỏ qua "BẮT BUỘC" sai của PM. Đối chiếu chi tiết: primary #1E5C49(v1.0)≠#0F7A56(v0.2) · ink/danger/info đều khác · font Be Vietnam Pro primary(v1.0) vs Inter primary+no-serif(v0.2) · radius 6/10(v1.0) vs 8/12(v0.2) · **"Hoàn thành=xám lặng"(v1.0) vs v0.2 map done→xanh** (PR #468 dùng xanh, khớp v0.2) · icon-ban tuyệt đối(v1.0) vs v0.2/#206 cho phép icon+label. **Action:** filed #469 (self-flag `spec-conflict`) — đề xuất v0.2 giữ canonical, 3 ý tưởng đáng giá từ v1.0 (done=xám lặng, serif hợp đồng, icon-ban) → soạn riêng thành **v0.3 delta** trình Kevin ratify sau, KHÔNG áp đặt ngay. Đính chính công khai trên #467. **Bài học quy trình:** LUÔN đọc DEC registry + design system hiện có trước khi propose bất kỳ chuẩn mới nào — kể cả khi "cảm thấy" đang làm điều đúng. **PR #468 review riêng:** chất lượng thực thi tốt (series bucket-placement logic thông minh, Q1-Q5 đúng #466, scope kỷ luật đúng DEC-047) — 1 gap thật cần Kevin/QC cân nhắc: bulk-complete không qua modal xác nhận D-02 (single-item OK theo pattern hiện có, nhưng bulk đổi nhiều nghĩa vụ pháp lý cùng lúc).
> **2026-07-02 (f):** **Servanda Design System v1.0 "Sổ cái" — Kevin RATIFIED.** PM build style guide sống (artifact tự render bằng token nó định nghĩa, 10 component HTML thật với data doc #14). Triết lý 5 nguyên tắc suy từ D-rules: sổ cái không phải dashboard startup · trạng thái không nói dối (D-08) · khẩn cấp phải "kiếm" (Von Restorff) · **serif = lời hợp đồng / sans = lời máy (D-06)** · xác nhận là nghi thức readback (D-02, không dark pattern D-11). Token: primary **Lục Khế #1E5C49** (giữ identity Khế), paper #FBFAF7, ink #1C2420, 4 semantic cố định (danger #A6372B độc quyền quá-hạn+phá-hủy · warn #8A5800 · info #33597E · done #5A6660 — hoàn thành = XÁM LẶNG không xanh ăn mừng). 8 luật màu cứng, 1 vùng đỏ/màn hình. Font: Be Vietnam Pro + Source Serif 4 self-host, thang 6 bậc, 3 weight, tabular-nums. **13 badge trạng thái chuẩn — không icon** (gồm Phạt viền-đỏ ≠ Quá hạn đỏ-đặc; Đã thanh lý cho backfill; Nhập tay/AI·đã duyệt cho 2 tier DEC-055). 11 định luật UI/UX kèm ràng buộc cụ thể = checklist QC verify mockup. Migrate: giữ tên token Tailwind hiện có, thay giá trị + dọn amber-*/red-* + xóa emoji JSX. **Relay:** chuẩn đầy đủ → [#467 comment](https://github.com/kevindo1103/khe/issues/467#issuecomment-4867720820) (Designer BẮT BUỘC theo) · gate note → #466 · DOCS_INBOX #1 posted (CLAUDE.md/SRS/Glossary fold). Dark mode = v2.
> **2026-07-02 (e):** **BA tab Nghĩa vụ & Quyền lợi ratified (Kevin, Q1-Q6) → #466 (parent) + #467 (Designer mockup) filed. Design-first gate: Designer → Kevin review → QC verify → CHỈ SAU ĐÓ file Backend/Frontend** — chưa file 2 lane này, tránh dev nhặt việc sớm trên core-value feature. **Q1 điều chỉnh quan trọng sau điều tra:** self-party per-tenant auto-map **ĐÃ BUILD SẴN** (DEC-030/#238) — `directions.py` dùng `TenantProfile.legal_name` (Settings) fuzzy-match `Party.name/role_label`, KHÔNG phải per-doc pick. Vấn đề thực = `DocumentDetail.tsx` còn `SelfPartyGate` dropdown per-doc **cũ, mâu thuẫn trực tiếp** với #238 → xóa, thay bằng link Settings khi direction vẫn NULL (D-13). Backend cần điều tra riêng (song song, không block mockup) vì sao doc #14 auto-map fail (legal_name chưa set? hay `Party.aliases` tenant_022/#364 chưa vào `_self_party_strings()`?). **Q2:** giữ nút Hoàn thành đơn, thêm multi-select + bulk action bar (không dùng lifecycle-auto-settle như PM đề xuất trước — user tự quyết). **Q3/Q4/Q5:** đồng ý nguyên đề xuất (series collapse, chờ kích hoạt tách section + penalty tag, amount display rule ẩn raw junk). **Q6:** tab doc trước, đơn vị nhỏ nhất trước dashboard 8am. **Icon rule mới — áp dụng TOÀN BỘ design system:** bỏ hết emoji rời (⚠️✓✅💰 hiện rải rác `SelfPartyGate`/`SelfPartyConfirmed`/`CompletenessBanner`), thay badge chữ+màu theo token — pattern "Quá hạn X ngày" hiện có làm mẫu chuẩn. Không riêng tab này — flag cho Designer áp dụng toàn brand refresh song song DEC-055.
> **2026-07-02 (d):** **DEC-055 RATIFIED (Kevin, chat) — Positioning & Brand. Issue #465 + DOCS_INBOX relay posted.** 3 quyết định gộp: (1) **Tên thương mại "Servanda"** (giải R-7) — từ *pacta sunt servanda*; "Khế" giữ codename nội bộ; điều kiện tiên quyết = Kevin verify NOIP nhóm 9/42 + domain servanda.vn/.com trước public use. Quá trình chọn: Pacta ☠️ (CLM Đức, DiliTrust mua 3/2025 — đụng ngành trực diện), Kept loại (VN khó đọc "-pt" + domain đắt), Kepta loại (Kapta CRM cách 1 nguyên âm), runner-up Corva/Covena/Keva. (2) **2-tier packaging:** Ledger/"Sổ" free (nhập tay obligations+parties+metadata, FULL core loop) + AI theo **quota per-DOCUMENT** (không per-tenant — tránh vách đá nâng cấp; quota infra tenant_004/005 có sẵn). Insight Kevin: bảng "suy giảm thiếu AI" = thang giá trị gia tăng của gói AI. Manual v1 KHÔNG có clause-tree editor (nhập tay tầng nghĩa vụ = 20% data mang 80% value). Chiến lược: sửa hình dạng GM DEC-011 (AI cost bọc trong quota, free marginal ≈ 0, kill-hedge DEC-002) · kênh firm TỰ upsell theo kinh tế lao động junior · alternative "người nhập liệu rẻ" thành user của mình (khớp DEC-012/D-03). Schema đã sẵn: `source="user_manual"` tenant_015. (3) **D-11 mới: không bao giờ paywall an toàn** (reminder/D-02/D-07/audit/consent) — chỉ monetize trí tuệ & tiện lợi. **OPEN:** OPEN-1 chat metadata-only tier nào · OPEN-2 tên tier UI (Designer). Build backlog file SAU docs fold (tôn trọng cascade). **Cùng ngày (chưa ratified):** BA tab Nghĩa vụ & Quyền lợi (6 pain points từ doc #14 staging: gate chồng gate, backfill "quá hạn 3027 ngày" = noise cần bulk-settle, series trải phẳng 14 hàng, temporal chưa là trục chính, amount_raw rác cạnh 💰, tab doc ≠ dashboard 8am) + IA 3 trục (direction × temporal × series) + Q1–Q6 chờ Kevin ratify — BA này giờ là mặt bằng hội tụ của CẢ 2 tier nên priority tăng.
> **2026-07-02 (c):** **#443 6 workstreams ĐÃ merge (WS0/WS1a/WS1b/WS3(#439)/WS2a/WS2b — PR #452/#458/#433+441/#456/#459) nhưng QC (#460) bắt gap: `two_pass_runner.py` build+test xong (23/23) nhưng KHÔNG được gọi từ `extraction_runner.py` MAX_TOKENS path — AC gốc "doc #20 no truncation" CHƯA đạt.** Verify trực tiếp `origin/staging`: `is_max_tokens_truncation()` → `_mark_transient_failure` → `retry_needed`, không có call `two_pass_runner`/`persist_skeleton`/`run_content_fill`. PR #459's author cố ý KHÔNG tự wire — đúng tinh thần Decision Review Gate (production-behavior decision → escalate, không tự assume). **Kevin ratified 3 câu hỏi #460:** (1) auto-trigger (không opt-in — opt-in tái tạo lỗ hổng "doc nằm im chờ người phát hiện" mà #443 sinh ra để fix), (2) background job nối vào extraction_progress có sẵn (không endpoint mới), (3) KHE_Backend owned (cùng lane #458/#459). **Bonus decision — Kevin hỏi "two-pass cho TẤT CẢ doc để đảm bảo integrity?"** → PM phân tích + Kevin ratify **KHÔNG**: two-pass không tăng integrity cho doc vốn đã đúng 1-call (chỉ fix lỗi output-truncation, correlate với size/verbosity), trong khi cost 2-5x LLM calls cho ~90%+ doc chưa từng vỡ token → GM hit thật (DEC-011). Thay vào đó **risk-based routing** (predict trước khi extract dùng data `candidates_token_count` từ #445, route thẳng two-pass nếu vượt ngưỡng ước lượng — tránh cả lãng phí 1-call-doomed VÀ tránh đánh thuế toàn hệ thống) = fast-follow P1, gate trên vài tuần data thật, KHÔNG guess constant. **2 issue mới:** #461 (wiring, P0, đóng #443 AC) · #462 (risk-based routing, P1, blocked on #445 data). #443 tracker + backlog comment đã update. Sprint CHƯA đóng — chờ #461 merge+verify staging.
> **2026-07-02 (b):** **#443 QC review folded (accepted in full).** Verified vs *fresh* `origin/staging` (spec 2026-07-02(a) built on stale `origin/main` — staging ahead via PR #433/#425/#441). Corrections: (1) **#447 WS3 closed as dup → #439** — `_parse_path` ĐÃ resolve "PHỤ LỤC A"→`PL-X` (PR #433); real gap = bare "Khoản N" collide Điều N (#439, two-part: prompt=AI + hierarchy=Backend). #439 giờ là prereq WS2. (2) **#446 WS1-Backend trap: #435 reextract endpoint = HARD prereq** (không chỉ "coordinate") — trap tắt vision self-heal, cần recovery path trước khi merge. (3) runner `extraction_runner.py:292` (`_parse_path(...) or replace("Điều ",...)`) mis-resolve phụ lục definitions → folded #439. (4) **R4b (#367) demoted stretch**, khỏi WS2 blocking AC. (5) Lane clarified: AI = `modules/extraction/providers/*` + `prompts.py`; Backend = `app/**`. +2 AC vào #449: regression docs #6–20 equivalent + cross-ref/definition resolution phải chạy SAU Pass 1 skeleton (full path map) không interleave Pass 2. **Revised map:** #444 · #445 · #446 (blocked #435) · **#439** (was #447) · #448 · #449. QC catch chuẩn — tránh WS3 assignee waste cycle trên solved code.
> **2026-07-02 (a):** **Mini-sprint #443 filed — Two-Pass Map-Reduce Extraction** (Kevin ratified). Root: large docs (doc #20, PL A–F + 19 Điều) hit Gemini `MAX_TOKENS` on LLM *output* (#442) — OCR input chunked nhưng extraction pass single-call. Durable fix = map-reduce với per-section durable checkpoint. **6 sub-issues** (DEC-047 lane-split): #444 WS0 OCR-text download (Backend, ship first) · #445 WS1-AI max_output_tokens+instrument · #446 WS1-Backend trap MAX_TOKENS→retryable (sequence #435) · #447 WS3 phụ lục hierarchy parse (prereq Pass 1) · #448 WS2-AI two-pass prompts · #449 WS2-Backend content_status state machine + resume. **Ratified decisions:** (1) trap MAX_TOKENS → retryable, KHÔNG fallthrough Gemini provider khác (same output wall) → doc #20 ngừng auto-recover qua vision, deterministic "chia nhỏ" msg thay thế — accepted trade; (2) two-pass thay "selective TOÀN VĂN" (giữ full verbatim + bounded); (3) single-oversized-clause fallback = **chia nhỏ theo đoạn** (LLM-cleaned, NOT coordinate-slice); (4) OCR-download = quick win độc lập. **Bonus:** Pass 1 hierarchy detect hồi sinh R4b (#367) multi-doc split free. Schema: `Clause.content_status` (skeleton/filled/truncated) + `Clause.title` NULL→placeholder + API `GET /documents/{id}/ocr-text` → **DOCS_INBOX sau merge**. Kevin sẽ báo teams vào check sub-issues. **Đã review trước đó:** #435 (Gemini 503 guard, PM review + relay Backend #403) + #442 (MAX_TOKENS, Gemini opinion synthesized). Sequencing: WS0+WS1 (với #435) → WS3 → WS2.
> **2026-06-29 (d):** **Sprint 2 KICKED OFF.** Team kickoff issues filed: #403 Backend (8 tasks: #274 StandingDeriver · #275 direction reminders · #277 chat reliability · #276 completeness · #105 PII consent · #308 obligation.source · #249 journey gate · #273 overdue-flip) · #404 Frontend (4 tasks: #398 relationship UI · #276 completeness UI · #249 journey counter · #273 badge) · #405 Designer (relationship link UI mockup, blocks FE #398). All P1 items have no sequential deps — parallelize freely. DOCS_INBOX relay posted for EPIC #362 fold. Docs folded (Kevin confirm). DEC-002 revision + DEC-011 GM re-model = PM tasks this sprint.
