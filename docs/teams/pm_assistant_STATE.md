...
| DEC-048 | **Obligation Fulfillment & Dependency Chain = core MVP feature (PROPOSED — chờ Kevin ratify final + QC affirm + Compliance P4).** Khế track fulfillment (ngày + N evidence docs + actor) + resolve dependent due-dates. **Scope line:** conjunction (AND) + chain IN; disjunction/conditional/min-OR OUT — chỉ deterministic. **Kevin decided BOTH (backfill+forward) 2026-06-26:** onboarding ghi cả mốc lịch sử đã fulfilled (anchor cho future) lẫn forward → fulfillment-capture = HARD pre-pilot, actor = operator-for-SME. **Persona split:** fulfillment = SME UI; chain wiring = operator UI. **Phasing:** 0a (single-FK reuse, zero-migration, pre-pilot: G1/G2 fix + backfill + waiting_trigger viz) → 0b (join tables `obligation_anchors`+`obligation_evidence` + conjunction, pilot-if-signal, additive-then-migrate) → Tier 1 AI assist (benchmark-gated) → Tier 2 smart. **Blocking pre-reqs:** P1 remap-preservation · P2 is_evidence skip-extraction · P3 actor-attribution · P4 evidence-PII/DEC-039 (Compliance). Resolver `obligation_chain.py` đã có (35 dòng, wired mark-done) nhưng neo `today` + single-anchor + G4 chưa populate (never resolved on real data). BA: **#297 consolidated v0.1** + file `BA_obligation_fulfillment_chain_v0.1.md` (delivered Kevin, DOCS_INBOX routed). Gated sau #277+#282. | **Proposed** (PM 2026-06-26, QC reviewed, Kevin BOTH) | 2026-06-26 |
