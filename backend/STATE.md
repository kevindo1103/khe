...
| 5b | Reminder service + Telegram + APScheduler | #62 (#26 PR-B, PR #66) | `windsurf/feat-backend-reminder-service` | ✅ **merged → staging** (`e0e92de`, #62 closed) — all 5 fixes verified (DEC-025 routing, retry on telegram.error.*, failed-retryable, test-guard), 93/93 |
...
| 6 | Chat query MVP (retrieve-only, D-08) | #27 (PR #68) | `windsurf/feat-backend-chat-query` | ✅ **merged → staging** (`5f3f635`, #27 closed) — both fixes verified (source provenance, single-doc fallback guard), 103/103. **M1 backend slice COMPLETE.** |
...
- **Infra #45** — ✅ CLOSED. PR #48 wired `migrate_all_tenants.py` + `CORS_ORIGINS=https://staging.khe.iceflow.cloud` + HTTPS-check. certbot/TLS live; deploy/nginx env-gate fixed (#70/#71/#72).
- **🟢 STAGING LIVE for UAT (2026-06-19)** — deploy-staging run #117 ✅ SUCCESS after hotfix **PR #73** (`0e28dd5`). Root cause of prior deploy failures (#115): VPS master.db was `create_all`'d without an `alembic_version` stamp → `alembic upgrade head` collided ("table tenants already exists"). Fix: idempotent master `001` (inspector skip-if-exists) + baseline-stamp in `migrate_all_tenants.py` + `_seed_uat_tenant()` in lifespan. **UAT creds:** tenant `uat-demo` / user `demo` / pwd `Khe@UAT2026` (env `UAT_DEMO_PASSWORD`), `vision_extraction` consent pre-granted. Admin web M0 (upload→list/detail→obligations) UAT-ready at `staging.khe.iceflow.cloud/admin/login`. PWA chat (#44) NOT merged (frontend `frontend/` structure collision w/ Admin + chat-shape adapter — flagged to PWA/Frontend leads).
- **🎉 UAT FIRST EXTRACTION ✅ 2026-06-19** — first real signed PDF (FRANKLINCOVEY ↔ PENFIELD training contract) extracted end-to-end on staging. status=extracted, 7 fields with per-field confidence (0.90–0.98), `doc_type=hd_nha_cung_cap` (classifier correct for HĐ dịch vụ đào tạo), **D-08 honored** (ngay_het_han=null + needs_review=true — not fabricated), **obligation_count=1** (engine derived a row from non-numeric `thoi_han_hd="4 buổi (16 giờ)"` — likely DEC-020 open_ended_review branch, awaiting confirmation). #75 §B+§C+§E pass on this contract. Resolution path #79: PR #80 (diagnostic) → #83 (failure_reason) → #84 (SDK packaging root cause). M0+M1 vertical slice (upload→extract→derive) verified.
- **#77 "CRITICAL get_db isolation bug" — VERIFIED MISDIAGNOSIS, downgraded.** `get_db` IS tenant-scoped (#9/#12 fix): reads `request.state.tenant_id` set by `get_current_user`; all 7 routers declare it before `get_db`; 6 existing cross-tenant tests pass in dev-env (where a leak would show). #77's proposed `Depends(get_tenant_session)` would make `tenant_id` a **query param** = broken auth-scoping → NOT applied. Added **PR #78** bidirectional 2-tenant HTTP isolation lock (merged `0670d2b`). Real residual = staging runs `ENVIRONMENT=development` → `get_db` dev-fallback active vs fail-closed 500; relayed to Infra #70 (`ENVIRONMENT=staging/production`). **QC §G NOT blocked.**
