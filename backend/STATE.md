...
- **#53** `for:ai` provider factory — contract APPROVED. ⚠️ **Merged to `main` not `staging`** (PR #55, `fd1edc2`) — verified `factory.py` 404 on staging + `__init__` lacks the export. **#25 PR-B (off staging) still blocked** → requested KHE_AI cherry-pick the factory (`factory.py` + `is_error` in `schemas.py` + `__init__` exports) onto a staging-based branch, PR base `staging`. Will sign-off + merge → then unblock PR-B. (Recurring "lands on main not staging" pattern — cf. Infra #48.)
