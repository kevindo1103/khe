...
| DEC-048 | **Obligation Fulfillment & Dependency Chain = core MVP feature (PROPOSED — QC review #297 → Kevin ratify).** Khế track fulfillment (ngày thực tế + N evidence docs) + resolve dependent due-dates. **Scope line:** conjunction (AND) + chain IN; disjunction/conditional (OR/IF) OUT — chỉ deterministic. **Manual-first:** UI editor = sàn (Tier 0 pre-pilot, AI off); AI assist (Tier 1) gated bằng benchmark; smart evidence (Tier 2) post-pilot. Open-ended partial delivery = manual only MVP. Data: `obligation_anchors` + `obligation_evidence` join tables + `fulfilled_at`. Resolver `obligation_chain.py` đã tồn tại (35 dòng, wired vào mark-done) nhưng neo `today` + single-anchor + chưa populate → cần rewrite multi-anchor + max-date. BA #297 (10 open Q cho QC). | **Proposed** (PM 2026-06-26) | 2026-06-26 |
