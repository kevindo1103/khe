...
*Branch: `claude/pm-assistant` | Last updated: 2026-06-26 | v3.6*
...
| **Backend** | **#302** | **[EPIC #300] Fulfillment capture + chain resolve** — mark-done (date+evidence_ids+actor), `waiting_trigger` resolve chain, backfill mốc lịch sử. **+Compliance P4 ACs:** `is_evidence→contains_personal_data=true` (default), purpose enum `obligation_fulfillment`, download gate (DEC-039), server-side skip-extraction trước `provider.extract()`. | **Pre-pilot 0a** |
