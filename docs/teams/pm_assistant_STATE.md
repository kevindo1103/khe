...
*Branch: `claude/pm-assistant` | Last updated: 2026-06-26 | v3.7*
...
| **ALL** | **#300** ✅ EPIC | **Document Detail Screen EPIC** — gom #281/#282/#284/#297 + §13 clause-edit re-read addendum. DEC-048 RATIFIED (Kevin 2026-06-26). QC §13 review 4 gap (G1/G2/G4 sập về **re-read=clause-scoped**, G3=stale banner). **Kevin Option B (2026-06-26):** `source_clause_num` PROMOTED fast-follow→**0a pre-pilot** → re-read trigger pre-pilot ngày 1 (gated P1+source_clause_num). Phasing 0a (pre-pilot: P1–P5 + G1/G2 fix + backfill + source_clause_num + clause-scoped re-derive + clause edit + re-read) → 0b (join tables + conjunction) → T1/T2 AI. Each team = own PR per lane (DEC-047). Sub-issues: #301–#307 + **#313**. | **Pre-pilot core 🔴** |
| **Backend** | **#301** 🔴 | **[EPIC #300] P1 — remap-preservation by `source`** — HARD BLOCKER. DELETE-ALL remap → source-aware merge: giữ `user_manual` + fulfilled obligations; chỉ update `ai_extracted` chưa đụng. Re-derived → `source=ai_re_derived` surface qua diff (D-02). Gates ALL re-derive/re-read paths + #303 FE nút. | **Gates #302/#303/#306 re-read** |
| **Backend** | **#302** | **[EPIC #300] Fulfillment capture + chain resolve** — mark-done (date+evidence_ids+actor), `waiting_trigger` resolve chain, backfill mốc lịch sử. **+Compliance P4 ACs.** **PR #311 ✅ APPROVED** (V1 fix `9921fe3` — derive delete path-2 `fulfilled_at.is_(None)` guard + test hardened; QC verified). **V2 gap → #313.** Clear to merge. | **PR #311 approved — pending merge** |
| **Backend** | **#303** | **[EPIC #300] `source_clause_num` column + Clauses API** — add `source_clause_num` to `obligations` (migration), expose `GET /documents/{id}/clauses`. Kevin Option B: promoted 0a. Gates clause↔obligation cross-nav + clause-scoped re-derive. | **0a (Option B — promoted pre-pilot)** |
| **KHE_AI** | **#304** | **[EPIC #300] Populate `source_clause_num` + clause-scoped re-derive** — derivation pipeline writes `source_clause_num` per obligation; re-read endpoint triggers text-only re-derive on 1 clause (not full doc vision). Gated P1 merge. | **Gates clause re-read** |
| **Designer** | **#305** | **[EPIC #300] Mockups: re-read banner + diff-confirm + stale-edit banner** — UX for: banner sau edit → "Khế đọc lại" nút (disabled pre-P1); diff-confirm per-obligation (D-02); stale-edit banner (G3 clause edited-after-re-read); `waiting_trigger` viz (chain 0a). Gates FE #306. | **Gates FE #306** |
| **Frontend** | **#306** | **[EPIC #300] Fulfillment capture + clause edit + re-read wiring** — fulfillment capture UI (date+evidence+actor), clause inline edit → `ClauseEditEvent`, re-read trigger (disabled/hidden gated P1+source_clause_num), diff-confirm D-02, stale-edit banner, `waiting_trigger` chip. | **Pre-pilot 0a** |
| **Compliance** | **#307** | **[EPIC #300] P4 — evidence doc PII + DEC-039 gate** — assess `is_evidence` docs: `contains_personal_data=true` default, DEC-039 firm-gate reuse (metadata-only), purpose enum `obligation_fulfillment` (storage-only, no consent gate, log Event). **✅ Assessment posted 2026-06-26** — Backend confirm P2 server-side enforcement pending. | **P4 ✅; Backend confirm P2 pending** |
| **Backend** | **#313** 🔴 | **[EPIC #300] P5(b) — cascade-past unfulfilled false-overdue + reminder spam (backfill)** — `propagate_obligation_done()` set cascade child `status=pending + due_date=past` (unfulfilled) → NOT caught by #311 P5 filter → false-overdue + reminder spam lúc onboard. **PM ratify: Option B** — cascade-past child `due_date<today` → `status=awaiting_confirmation` (D-02, "cần xác nhận đã hoàn thành?"). Loại khỏi overdue flip + reminder. Filed 2026-06-26 (QC relay #311 V2). | **Pre-pilot 🔴 — phải đóng trước onboard 20 SME** |
