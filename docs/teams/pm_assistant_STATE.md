...
*Branch: `claude/pm-assistant` | Last updated: 2026-06-20 | v1.7*
...
| DEC-030 | **Obligation direction + Quyền lợi + self-party.** Mỗi Obligation có `direction` = `nghĩa_vụ` (mình phải làm) / `quyền_lợi` (đối tác phải làm cho mình) / `null` (cần user confirm — D-02). `obligor` = role_label bên phải thực hiện (từ parties[].payer). Cross-team data contract: KHE_AI thêm `parties[] = {name, role_label}` + `payment_schedule[].payer`; Backend thêm `direction` + `obligor` columns + `legal_name` tenant profile + auto-match logic; Frontend thêm 2-tab UI (Nghĩa vụ/Quyền lợi) + confirmation screen. **#122 RESOLVED** via Option B: rename `obligation_type` cadence → `recurrence`; `obligation_type` = category (DEC-027 values). D-06 ✅ D-02 ✅ D-08 ✅. Issues: #144 (KHE_AI) · #145 (Backend) · #146 (Frontend). | **Ratified** (Kevin 2026-06-20) | 2026-06-20 |
