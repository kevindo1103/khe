...
function StatusPill({ doc, docId }: { doc: DocumentListItem; docId?: number }) {
  ...
  return (
    <span className="inline-flex items-center" data-testid={docId != null ? `doc-status-${docId}` : undefined}>
...
function DocCard({ doc, onClick }: { doc: DocumentListItem; onClick: () => void }) {
  return (
    <div
      ...
      aria-label={`Mở chi tiết hợp đồng ${docTypeLabel(doc.doc_type)}${doc.primary_party ? ` với ${doc.primary_party}` : ''}`}
      data-testid={`doc-row-${doc.id}`}
      data-status={doc.status}
      className="px-4 py-3 border-b border-border last:border-0 cursor-pointer hover:bg-surface-alt active:bg-surface-alt transition-colors focus-visible:outline-none focus-visible:shadow-ring"
    >
      {/* Title + status pill */}
    ...
    <div>
      ...
      >
        <div className="flex gap-2 pb-1 md:flex-wrap">
          {/* CR11: hide pending chip when count = 0 */}
          {counts.pending > 0 && (
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              <tr
                ...
                role="button"
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/admin/documents/${doc.id}`); } }}
                aria-label={`Mở chi tiết hợp đồng ${docTypeLabel(doc.doc_type)}${doc.primary_party ? ` với ${doc.primary_party}` : ''}`}
                data-testid={`doc-row-${doc.id}`}
                data-status={doc.status}
                className="cursor-pointer hover:bg-surface-alt transition-colors border-b border-border last:border-0 group focus-visible:outline-none focus-visible:shadow-ring"
              >
                {/* Col 1 — Hợp đồng */}
