...
_Last updated: 2026-06-20 (#146 PR #177 open — chat obligation sources enrichment + waiting_trigger fix)_
...
**Status:** 12 PRs on `staging` + PR #177 (#146) in review. Batch promote to `main` ready — optional gate (#163) cleared.
...
| # | Title | Status | Notes |
|---|-------|--------|-------|
| #155 | DEC-030 self-party endpoints — legal_name + confirm_self_party + parties[] persist | assigned → Windsurf_Backend | Gates FE #158 (HIGH). Plan posted [#155 comment](https://github.com/kevindo1103/khe/issues/155). Migration `tenant_008` (extend existing `parties` table — +`document_id`/`role_label`). Contract frozen + relayed to #158 (FE soft-unblocked, building vs mocks). Branch `windsurf/feat-backend-self-party-endpoints` off staging. |
| #146 | Chat obligation sources: direction/series fields + waiting_trigger fix | PR #177 open (review pending) | No migration. chat_query.py only. |
...
| #164 | Chat tokenomics: token + cost tracking on ChatQueryLog | `status:done-staging` ✅ | PR #169 merged. Migration `tenant_007`. DOCS_INBOX posted 2026-06-20. |
| #146 | Chat obligation sources: direction/series fields + waiting_trigger | PR #177 open | No migration. chat_query.py: 9 new fields in sources + skip-guard fix. |
