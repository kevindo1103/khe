...
| **Backend** | #270 (BA) | Firm portal engineering BA (canonical) — cross-tenant fan-out + consent state machine + DEC-039 visibility. **Q0=B → pre-pilot (DEC-042)** | — |
| **Backend** | #65 | Firm portal scaffold — **UNBLOCKED + assigned** (dep #63 merged). Architecture = BA #270 | — |
| **Backend** | #237 | Firm portal Phase B+C (handshake + portfolio + lead signals) — sequenced after #65 | #65 merge + `contains_personal_data` |
| **KHE_AI** | (in #237) | Set `contains_personal_data` at extraction (`doc_type_group=='lao_dong'`) + per-tenant backfill | dep of #237 Phase C |
